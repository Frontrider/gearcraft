package hu.frontrider.gearcraft.core.util.items
...
fun IItemHandler.extractFirst(doExtract: Boolean,count:Int=0): ItemStack {
    ...
    for (i in 0 until this.slots) {
        ...
        if (itemStack.isNotEmpty()) {
            ...
            return this.extractItem(i, stackCount, doExtract)
        }
...
fun IItemHandler.extract(stack: ItemStack,startIndex: Int=0, endIndex: Int=this.slots-1){
    for (i in startIndex..endIndex) {
        val inventoryStack = this.getStackInSlot(i)
        if (inventoryStack.sameAs(stack)) {
                this.extractItem(i,stack.count,false)
        }
    }
}